...
// All logic lives in useProfileViewModel.
...
const THUMB_SIZE   = (SCREEN_WIDTH - 48 - 8 * 2) / 3; // 3 per row, 24px card padding each side
...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
        ...
        {myListings.length === 0 ? (
          <View style={styles.emptyListingsBox}>
            ...
            <Text style={styles.emptyListingsText}>
              Your posted listings will show here.
            </Text>
          </View>
        ) : (
          myListings.map((item) => (
      ...
      {/* ── Create listing ───────────────────────────────────────────────── */}
      <View style={styles.card}>
        <Text style={styles.cardTitle}>Post a Listing</Text>
        ...
        {/* Photo thumbnails grid with remove buttons */}
        ...
        {/* Empty state when no photos picked yet */}
        {listingImages.length === 0 && (
          <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  email: { fontSize: 14, color: "#7b8794", marginBottom: 28 },
  ...
  myListingRow:      { flexDirection: "row", alignItems: "center", marginBottom: 14, position: "relative" },
